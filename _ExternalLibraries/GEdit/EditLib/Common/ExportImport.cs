...
namespace EditLib
{
    public static class ExportImport
    {
        ...
        {
            var x = layer.Figures.Select(item => item.GetTransformedPath().Path.GetBounds()).Min(item => item.Left);
            var y = layer.Figures.Select(item => item.GetTransformedPath().Path.GetBounds()).Min(item => item.Top);
            var bounds = new RectangleF();
            foreach (var rect in layer.Figures.Select(item => item.GetTransformedPath().Path.GetBounds()))
                bounds = RectangleF.Union(bounds, rect);
            var width = (int)(bounds.Width + x);
            var height = (int)(bounds.Height + y);
            var format = GetFileFormat(fileName);
            SaveImage(fileName, layer, width, height, format);
        }

        private static void SaveImage(string fileName, Layer layer, int width,
            int height, ImageFormat format)
        {
            using (var image = new Bitmap(width, height))
            {
                using (var g = Graphics.FromImage(image))
                {
                    ...
                    else if (format == ImageFormat.Jpeg || format == ImageFormat.Gif)
                        g.Clear(Color.White);
                    // отрисовка созданных фигур
                    foreach (var fig in layer.Figures.Where(fig => layer.IsVisible(fig)))
                        fig.Renderer.Render(g, fig);
                }
