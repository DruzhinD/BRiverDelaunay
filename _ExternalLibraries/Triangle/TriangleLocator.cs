...
namespace TriangleNet
{
    ...
    /// <примечания>
    /// ПРЕДУПРЕЖДЕНИЕ: 
    /// Эта процедура предназначена для выпуклой триангуляции и будет
    /// работать после того, как отверстия и вогнутости будут вырезаны.
    /// На основе статьи Эрнста П. Мукке, Исаака Сайаса и Биньхая Чжу "Быстрый
    /// Рандомизированное местоположение точки Без предварительной обработки в Двух- и трехмерном
    /// Триангуляции Делоне", Материалы Двенадцатого ежегодного симпозиума по
    /// Вычислительной геометрии, ACM, май 1996 года.
    /// </примечания>
    public class TriangleLocator
    {
        ...
        public TriangleLocator(IMeshNet mesh)
            : this(mesh, RobustPredicates.Default)
        {
        }

        public TriangleLocator(IMeshNet mesh, IPredicates predicates)
        {
            this.mesh = mesh;
            ...
            bool stopatsubsegment)
        {
            ...
            while (true)
            {
                ...
                if (mesh.checksegments && stopatsubsegment)
                {
                    ...
                    if (checkedge.seg.hash != IMeshNet.DUMMY)
                    {
                ...
                if (searchtri.tri.id == IMeshNet.DUMMY)
                {
                    // Go back to the last triangle.
                    backtracktri.Copy(ref searchtri);
                    return LocateResult.Outside;
