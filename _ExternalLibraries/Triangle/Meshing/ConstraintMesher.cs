...
namespace TriangleNet.Meshing
{
    ...
    internal class ConstraintMesher
    {
        ...
        public ConstraintMesher(TriangleNet.IMeshNet mesh, Configuration config)
        {
            this.mesh = mesh;
        ...
        private void CarveHoles()
        {
            ...
            if (regionTris != null)
            {
                ...
                {
                    if (regionTris[i].id != TriangleNet.IMeshNet.DUMMY)
                    {
        ...
        private void InfectHull()
        {
            ...
            do
            {
                ...
                if (!hulltri.IsInfected())
                {
                    ...
                    if (hullsubseg.seg.hash == TriangleNet.IMeshNet.DUMMY)
                    {
                ...
                while (nexttri.tri.id != TriangleNet.IMeshNet.DUMMY)
                {
        ...
        void Plague()
        {
            ...
            for (int i = 0; i < viri.Count; i++)
            {
                ...
                for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
                {
                    ...
                    if ((neighbor.tri.id == TriangleNet.IMeshNet.DUMMY) || neighbor.IsInfected())
                    {
                        if (neighborsubseg.seg.hash != TriangleNet.IMeshNet.DUMMY)
                        {
                            // There is a subsegment separating the triangle from its
                            // neighbor, but both triangles are dying, so the subsegment
                            // dies too.
                            mesh.SubsegDealloc(neighborsubseg.seg);
                            if (neighbor.tri.id != TriangleNet.IMeshNet.DUMMY)
                            {
            ...
            foreach (var virus in viri)
            {
                ...
                for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
                {
                    ...
                    if (testvertex != null)
                    {
                        ...
                        while ((neighbor.tri.id != TriangleNet.IMeshNet.DUMMY) &&
        ...
        private FindDirectionResult FindDirection(ref Otri searchtri, Vertex searchpoint)
        {
            ...
            if (leftflag && rightflag)
            {
                ...
                if (checktri.tri.id == TriangleNet.IMeshNet.DUMMY)
                {
                    leftflag = false;
                }
                else
            ...
            while (leftflag)
            {
                ...
                searchtri.Onext();
                if (searchtri.tri.id == TriangleNet.IMeshNet.DUMMY)
                {
        ...
        private void SegmentIntersection(ref Otri splittri, ref Osub splitsubseg, Vertex endpoint2)
        {
            ...
            do
            {
                ...
                splitsubseg.Next();
            } while (splitsubseg.seg.hash != TriangleNet.IMeshNet.DUMMY);
            do
            {
                opposubseg.SetSegOrg(newvertex);
                opposubseg.Next();
            } while (opposubseg.seg.hash != TriangleNet.IMeshNet.DUMMY);
        ...
        private bool ScoutSegment(ref Otri searchtri, Vertex endpoint2, int newmark)
        {
            ...
            else
            {
                ...
                if (crosssubseg.seg.hash == TriangleNet.IMeshNet.DUMMY)
                {
                    return false;
                }
                else
        ...
        private void DelaunayFixup(ref Otri fixuptri, bool leftside)
        {
            ...
            if (fartri.tri.id == TriangleNet.IMeshNet.DUMMY)
            {
                return;
            }
            neartri.Pivot(ref faredge);
            if (faredge.seg.hash != TriangleNet.IMeshNet.DUMMY)
            {
                return;
            }
