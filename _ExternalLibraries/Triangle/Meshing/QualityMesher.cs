...
namespace TriangleNet.Meshing
{
    ...
    class QualityMesher
    {
        ...
        public QualityMesher(TriangleNet.IMeshNet mesh, Configuration config)
        {
            logger = Log.Instance;
        ...
        public int CheckSeg4Encroach(ref Osub testsubseg)
        {
            ...
            if (neighbortri.tri.id != TriangleNet.IMeshNet.DUMMY)
            {
                sides++;
        ...
        public void TestTriangle(ref Otri testtri)
        {
            ...
            if ((angle > behavior.goodAngle) || (maxangle < behavior.maxGoodAngle && behavior.MaxAngle != 0.0))
            {
                    ...
                    (base2.type == VertexType.SegmentVertex))
                {
                    ...
                    if (testsub.seg.hash == TriangleNet.IMeshNet.DUMMY)
                    {
                        ...
                        do
                        {
                            tri1.Oprev();
                            tri1.Pivot(ref testsub);
                        } while (testsub.seg.hash == TriangleNet.IMeshNet.DUMMY);
        ...
        private void SplitEncSegs(bool triflaws)
        {
            ...
            while (badsubsegs.Count > 0)
            {
                ...
                if (!Osub.IsDead(currentenc.seg) && (eorg == seg.org) && (edest == seg.dest))
                {
                    ...
                    acuteorg = testsh.seg.hash != TriangleNet.IMeshNet.DUMMY;
                    // Is the destination shared with another segment?
                    testtri.Lnext();
                    testtri.Pivot(ref testsh);
                    acutedest = testsh.seg.hash != TriangleNet.IMeshNet.DUMMY;
                    ...
                    if (testtri.tri.id != TriangleNet.IMeshNet.DUMMY)
                    {
                        // Is the destination shared with another segment?
                        testtri.Lnext();
                        testtri.Pivot(ref testsh);
                        acutedest2 = testsh.seg.hash != TriangleNet.IMeshNet.DUMMY;
                        acutedest = acutedest || acutedest2;
                        // Is the origin shared with another segment?
                        testtri.Lnext();
                        testtri.Pivot(ref testsh);
                        acuteorg2 = testsh.seg.hash != TriangleNet.IMeshNet.DUMMY;
