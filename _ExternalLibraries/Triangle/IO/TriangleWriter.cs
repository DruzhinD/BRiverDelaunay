...
namespace TriangleNet.IO
{
    ...
    public class TriangleWriter
    {
        ...
        /// <param name="mesh"></param>
        /// <param name="filename"></param>
        public void WriteNodes(IMeshNet mesh, string filename)
        {
        ...
        /// Number the vertices and write them to a .node file.
        /// </summary>
        private void WriteNodes(StreamWriter writer, IMeshNet mesh)
        {
        ...
        /// <param name="mesh"></param>
        /// <param name="filename"></param>
        public void WriteElements(IMeshNet mesh, string filename)
        {
        ...
        /// <param name="mesh"></param>
        /// <param name="filename"></param>
        public void WritePoly(IMeshNet mesh, string filename)
        {
        ...
        /// make sure a .node file was written before, so that the nodes 
        /// are numbered right.</remarks>
        public void WritePoly(IMeshNet mesh, string filename, bool writeNodes)
        {
        ...
        /// <param name="mesh"></param>
        /// <param name="filename"></param>
        public void WriteEdges(IMeshNet mesh, string filename)
        {
            ...
            using (var writer = new StreamWriter(filename))
            {
                ...
                foreach (var item in mesh.triangles)
                {
                    ...
                    {
                        tri.Sym(ref trisym);
                        if ((tri.tri.id < trisym.tri.id) || (trisym.tri.id == IMeshNet.DUMMY))
                        {
                            p1 = tri.Org();
                            p2 = tri.Dest();
                            ...
                            if (behavior.UseBoundaryMarkers)
                            {
                                ...
                                if (behavior.useSegments)
                                {
                                    ...
                                    if (checkmark.seg.hash == IMeshNet.DUMMY)
                                    {
                                ...
                                {
                                    writer.WriteLine("{0} {1} {2} {3}", index, p1.id, p2.id,
                                            trisym.tri.id == IMeshNet.DUMMY ? "1" : "0");
                                }
                            }
                            else
                            {
        ...
        public void WriteNeighbors(IMeshNet mesh, string filename)
        {
            Otri tri = default(Otri), trisym = default(Otri);
            int n1, n2, n3;
            int i = 0;
