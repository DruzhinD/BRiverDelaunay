...
namespace TriangleNet.Voronoi.Legacy
{
    ...
    public class BoundedVoronoiLegacy : IVoronoi
    {
        ...
        public BoundedVoronoiLegacy(IMeshNet mesh)
            : this(mesh, true)
        {
        }
        ...
        /// </summary>
        /// <param name="mesh">Mesh instance.</param>
        public BoundedVoronoiLegacy(IMeshNet mesh, bool includeBoundary)
        {
            this.mesh = mesh;
        ...
        private void TagBlindTriangles()
        {
            ...
            foreach (var ss in mesh.subsegs.Values)
            {
                ...
                if (f.tri.id != IMeshNet.DUMMY && !f.tri.infected)
                {
                ...
                while (triangles.Count > 0)
                {
                    ...
                    if (TriangleIsBlinded(ref f, ref e))
                    {
                        ...
                        for (f.orient = 0; f.orient < 3; f.orient++)
                        {
                            ...
                            if (f0.tri.id != IMeshNet.DUMMY && !f0.tri.infected && sub1.seg.hash == IMeshNet.DUMMY)
                            {
        ...
        private void ConstructBoundaryCell(Vertex vertex)
        {
            ...
            if (f_prev.tri.id != IMeshNet.DUMMY)
            {
                // Go clockwise until we reach the border (or the initial triangle)
                while (f_prev.tri.id != IMeshNet.DUMMY && !f_prev.Equals(f_init))
                {
                    f_prev.Copy(ref f);
                    f_prev.Oprev();
                }
            ...
            do
            {
                ...
                if (f_next.tri.id == IMeshNet.DUMMY)
                {
                    if (!f.tri.infected)
                    {
