...
namespace TriangleNet
{
    ...
    /// Реализованные здесь адаптивные точные арифметические геометрические предикаты описаны в
    /// подробно в статье «Адаптивная точная арифметика с плавающей запятой и быстрая надежность.
    /// Геометрические предикаты. »Джонатан Ричард Шевчук, см.
    /// http://www.cs.Cmu.edu/~quake/robust.html
    ...
    /// Макросы исходного кода C были автоматически расширены с помощью Visual Studio
    /// командная строка с командой «CL / P / C EXACT.C», см.
    /// http://msdn.microsoft.com/en-us/library/8z9z0bx6.aspx 
    /// =======================================================
    /// Надежные геометрические предикаты 
    /// =======================================================
    /// </remarks>
    public class RobustPredicates : IPredicates
    {
        ...
        /// <summary>
        /// Проверяем, появляются ли три точки против часовой стрелки. Результат
        /// также грубое приближение удвоенной площади со знаком определяемого треугольника
        /// по трем точкам.
        /// </summary>
        /// <param name = "pa"> Точка а. </param>
        /// <param name = "pb"> Пункт б. </param>
        /// <param name = "pc"> Пункт c. </param>
        /// <returns> Возвращает положительное значение, если точки pa, pb и pc встречаются в
        /// против часовой стрелки; отрицательное значение, если они идут по часовой стрелке;
        /// и ноль, если они коллинеарны. </returns> 
        ...
        /// <summary>
        /// Проверяем, лежит ли точка pd внутри круга, проходящего через pa, pb и pc. В
        /// точки pa, pb и pc должны быть в порядке против часовой стрелки, иначе это знак результата
        /// будет обратным.
        /// </summary>
        /// <param name = "pa"> Точка а. </param>
        /// <param name = "pb"> Пункт б. </param>
        /// <param name = "pc"> Пункт c. </param>
        /// <param name = "pd"> Точка d. </param>
        /// <returns> Возвращает положительное значение, если точка pd лежит внутри круга, через который проходит
        /// pa, pb и pc; отрицательное значение, если оно лежит снаружи; и ноль, если четыре точки
        /// кокциркулярны. </returns>
        ...
        /// are cocircular.</returns>
        /// <summary>
        /// Вернуть положительное значение, если точка pd несовместима с кругом
        /// или плоскость, проходящая через pa, pb и pc (это означает, что pd находится внутри
        /// круг или ниже плоскости); отрицательное значение, если оно совместимо; и
        /// ноль, если четыре точки кокружны / компланарны. Точки pa, pb и
        /// компьютер должен быть в порядке против часовой стрелки, иначе знак результата будет
        /// в обратном порядке.
        /// </summary>
        /// <param name = "pa"> Точка а. </param>
        /// <param name = "pb"> Пункт б. </param>
        ...
        /// <param name = "offconstant"> Константа вне центра. </param>
        /// <returns> Координаты окружности (или вне центра) </returns>
        ...
        /// <param name = "xi"> Относительная координата нового местоположения. </param>
        /// <param name = "eta"> Относительная координата нового местоположения. </param>
        /// <returns> Координаты центра окружности </returns>
        /// <примечания>
        /// Результат возвращается как в координатах x-y, так и в xi-eta
        /// (барицентрические) координаты. Система координат xi-eta определена в
        /// члены треугольника: начало треугольника является началом
        /// система координат; цель треугольника - одна единица вдоль
        /// ось xi; а вершина треугольника - одна единица по оси эта.
        /// Эта процедура также возвращает квадрат длины треугольника.
        /// кратчайшее ребро.
        /// </remarks>
        public Point FindCircumcenter(Point org, Point dest, Point apex,
        ...
        #region Exact arithmetics  Точная арифметика
